...
// These were legitimate original schools accidentally deleted by the cleanup script
const TO_RESTORE = [243744, 191515, 193654, 214713, 209825, 220613, 433660];

const FIELDS = [
  'id','school.name','school.city','school.state','school.control',
  'school.predominant_degree','school.school_url',
...
console.log(`Current count: ${colleges.length}`);
console.log(`\nRestoring ${TO_RESTORE.length} accidentally deleted schools...\n`);

const restored = [];
for (const ipeds of TO_RESTORE) {
  ...
  try {
    ...
    console.log(`  ✓ Restored: ${record.name} (${ipeds})`);
  } catch (err) {
    console.error(`  ✗ Error for IPEDS ${ipeds}:`, err.message);
  }
}

const updated = [...colleges, ...restored];
writeFileSync(DATA_PATH, JSON.stringify(updated, null, 2), 'utf8');

console.log(`\n✅ Done. Restored ${restored.length} schools. Total now: ${updated.length}`);
